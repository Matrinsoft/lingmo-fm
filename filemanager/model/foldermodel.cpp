...
void FolderModel::paste()
{
    ...
    if (enable) {
        ...
        if (mimeData->hasFormat("application/x-lingmo-cutselection")) {
...
void FolderModel::cut()
{
    ...
    mimeData->setData("application/x-lingmo-cutselection", QByteArray("1"));
...
void FolderModel::setWallpaperSelected()
{
    ...
    QDBusInterface iface("org.lingmo.Settings", "/Theme",
                         "org.lingmo.Theme",
...
void FolderModel::openContextMenu(QQuickItem *visualParent, Qt::KeyboardModifiers modifiers)
{
    ...
    if (indexes.isEmpty()) {
        ...
        selectAll->setIcon(QIcon::fromTheme("edit-select-all")); // 娣诲姞鍏ㄩ€夊浘鏍?
        ...
        if (!isTrash) {
            ...
            newMenu->setIcon(QIcon::fromTheme("document-new")); // 娣诲姞鏂板缓鏂囨。鍥炬爣
        ...
        menu->addSeparator();
        if (m_isDesktop) {
            menu->addAction(refresh);  // 娣诲姞鍒扮┖鐧藉尯鍩熺殑鍙抽敭鑿滃崟
        }
...
{
    if (!url.isEmpty()) {
        QProcess::startDetached("lingmo-filemanager", QStringList() << url);
        return;
    }

    // url 涓虹┖鍒欐墦寮€宸查€夋嫨鐨?items.
    if (!m_selectionModel->hasSelection())
        return;
    ...
    for (const QModelIndex &index : m_selectionModel->selectedIndexes()) {
        KFileItem item = itemForIndex(index);
        if (item.isDir()) {
            QProcess::startDetached("lingmo-filemanager", QStringList() << item.url().toLocalFile());
        }
    }
}
...
void FolderModel::createActions()
{
    ...
    connect(sortByName, &QAction::triggered, this, [=] {
        emit changeSortMode(0); // 鎸夋枃浠跺悕鎺掑簭
    });

    connect(sortByType, &QAction::triggered, this, [=] {
        emit changeSortMode(1); // 鎸夋枃浠剁被鍨嬫帓搴?
    });

    connect(sortByModified, &QAction::triggered, this, [=] {
        emit changeSortMode(2); // 鎸変慨鏀规椂闂存帓搴?
    });

    connect(sortBySize, &QAction::triggered, this, [=] {
        emit changeSortMode(3); // 鎸夋枃浠跺ぇ灏忔帓搴?
    });
